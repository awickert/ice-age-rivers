...
def resample_ice(self):
  ...
  if isll:
    ...
    for i in range(len(self.ice)):
      print self.ages[i]
      grass.run_command('r.resamp.interp', input=self.ice_raw_import[i], output=self.ice[i], overwrite=True)
...
def runoff_input_ice(self):
  ...
  for i in range(len(self.ages)-2):
    ...
    # If-statement if not actually ending at 0...
    if self.ages_numeric[i+1] == 0:
